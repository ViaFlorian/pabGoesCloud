...
public interface ErgebnisB007PdfRepository extends
    CrudRepository<ErgebnisB007Pdf, Long> {
  ...
  @Query("""
      ...
      resultat AS (
          SELECT
              vereinigtGefiltert.MitarbeiterID,
              vereinigtGefiltert.Kostenart,
              vereinigtGefiltert.KostenStunden,
              vereinigtGefiltert.Kostensatz,
              vereinigtGefiltert.Kosten,
              vereinigtGefiltert.LeistungStunden,
              vereinigtGefiltert.Stundensatz,
              vereinigtGefiltert.BerechneteLeistung,
              CASE
                  WHEN Projekt.Projekttyp = 'Dienstleistung' THEN vereinigtGefiltert.BerechneteLeistung
                  ELSE vereinigtGefiltert.FaktLeistung
              END AS FaktLeistung,
              vereinigtGefiltert.Buchungstyp,
              vereinigtGefiltert.Jahr,
              vereinigtGefiltert.Monat,
              Kunde.Kurzbezeichnung,
              Projekt.Projektnummer,
              Projekt.Bezeichnung,
              CASE
                  WHEN Mitarbeiter.ID IS NULL THEN 'ohne Mitarbeiterbezug'
                  WHEN Mitarbeiter.IstIntern = 1 THEN CONCAT(Mitarbeiter.Nachname, ', ', Mitarbeiter.Vorname)
                  ELSE CONCAT(
              ...
              END AS MitarbeiterName,
              Mitarbeiter.PersonalNr AS PersonalNr,
              Projekt.Projekttyp AS Projekttyp,
              NULL AS ReferenzMonat,
              NULL AS ReferenzJahr,
              Projektabrechnung.Fertigstellungsgrad AS Fertigstellungsgrad,
              CASE
                  WHEN AbgeschlosseneMonate.Jahr IS NULL THEN 1
                  ELSE 0
              END AS Vorlaeufig
          FROM
              vereinigtGefiltert
              ...
              Projektabrechnung ON
                  vereinigtGefiltert.ProjektID = Projektabrechnung.ProjektID
                  AND vereinigtGefiltert.Jahr = Projektabrechnung.Jahr
                  AND vereinigtGefiltert.Monat = Projektabrechnung.Monat
                  LEFT OUTER JOIN
              AbgeschlosseneMonate ON
                  vereinigtGefiltert.Jahr = AbgeschlosseneMonate.Jahr
                  AND vereinigtGefiltert.Monat = AbgeschlosseneMonate.Monat
                  LEFT OUTER JOIN
              Projekt ON Projektabrechnung.ProjektID = Projekt.ID
                  LEFT OUTER JOIN
              Kunde ON
                  Projekt.KundeID = Kunde.ScribeID
                  LEFT OUTER JOIN
              Organisationseinheit ON
                  Projekt.OrganisationseinheitID = Organisationseinheit.ScribeID
                  LEFT OUTER JOIN
              Mitarbeiter ON
                  Mitarbeiter.ID = vereinigtGefiltert.MitarbeiterID
          WHERE
      ...
      ORDER BY
          ...
          Kostenart ASC
                       """)
  List<ErgebnisB007Pdf> ladeB007Pdf(final int abJahr, final int abMonat,
